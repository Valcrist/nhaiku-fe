    ...
    <div class="bg-[#1c1f2e] rounded-lg p-6 flex gap-6">
      <div class="shrink-0">
        <button
          ...
          class="cursor-pointer p-0 border-0 bg-transparent block"
        >
          {#if manga.coverUrl}
            <img
              src={manga.coverUrl}
          ...
          {:else}
            <div
              class="w-48 h-72 bg-zinc-800 rounded flex items-center justify-center text-zinc-600 text-sm"
            >
              No cover
            </div>
          {/if}
      ...
      <div class="flex-1 min-w-0">
            ...
            <div class="flex gap-2 items-start">
              ...
              <div class="flex gap-1.5 flex-wrap flex-1">
                {#each tags as tag (tag.id)}
                  <span class="bg-zinc-700 text-zinc-200 px-2 py-0.5 rounded text-xs"
                    >{tag.slug}</span
                  >
                {/each}
              </div>
        ...
        <div class="mt-6 flex gap-3">
          <button
            class="flex items-center gap-1.5 bg-rose-700 hover:bg-rose-600 text-white text-sm px-4 py-1.5 rounded transition-colors cursor-pointer"
          >
            ♥ Votes ({manga.votes})
          </button>
          <button
            class="flex items-center gap-1.5 bg-zinc-700 hover:bg-zinc-600 text-white text-sm px-4 py-1.5 rounded transition-colors cursor-pointer"
          >
            ⬇ Re-index
          </button>
        </div>
